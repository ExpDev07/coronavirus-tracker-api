...
def test_requirements_txt():
    ...
    return_code_0 = subprocess.call("mkdir -p {}".format(temp_output_dir),
                                    shell=True)
    return_code_1 = subprocess.call("pipenv lock -r \
                                    > {}".format(req_test_file_path),
                                    shell=True)

    return_code_2 = subprocess.call("pipenv lock -r --dev \
                                    > {}".format(req_dev_test_file_path),
                                    shell=True)
    ...
    return_code_z = subprocess.call("rm -rf {}".format(temp_output_dir),
                                    shell=True)
