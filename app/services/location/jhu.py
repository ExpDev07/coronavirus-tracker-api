...
class JhuLocationService(LocationService):
    ...
    async def get_all(self):
        # Get the locations.
        locations = await JHUGateway.get_instance().get_locations()
        return locations
...
class JHUGateway(LocationGateway):
    __instance = None

    @staticmethod
    def get_instance():
        if JHUGateway.__instance is None:
            JHUGateway.__instance = JHUGateway()
        return JHUGateway.__instance


    def __init__(self):
        self.BASE_URL = "https://raw.githubusercontent.com/CSSEGISandData/2019-nCoV/master/csse_covid_19_data/csse_covid_19_time_series/"
    ...
    async def get_category(self, category):
        ...
        if cache_results:
            ...
            url = self.BASE_URL + "time_series_covid19_%s_global.csv" % category

            # Request the data
            LOGGER.info(f"{data_id} Requesting data...")
            async with ProcessSystem.get_instance().get_client_session().get(url) as response:
    ...
    async def get_locations(self):
        ...
        confirmed = await self.get_category("confirmed")
        deaths = await self.get_category("deaths")
        recovered = await self.get_category("recovered")
        ...
        for index, location in enumerate(locations_confirmed):
            ...
            timelines = {
                ...
                "deaths": self.parse_history(key, locations_deaths, index),
                "recovered": self.parse_history(key, locations_recovered, index),
            }
    ...
    def parse_history(self, key: tuple, locations: list, index: int):
        """
